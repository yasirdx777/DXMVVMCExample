...
final class CatsViewController: UITableViewController {
    ...
    func setupBindings() {
        viewModel.success.drive(onNext: {[weak self] (dataResponse: [[CatsPost]]) in
            if dataResponse.isEmpty {return}
            let dataSources:[UITableViewDataSource] = [CatsListDataSource(posts: dataResponse[0]), KittensListDataSource(posts: dataResponse[1])]
